...
// recursive method to extract the root error
// Imagine we have A(A(A(A(error)))), after each iteration, we extract A because
//A is AppError type, until we get the last error (not AppError type), we return it
...
//Only in small project, not recommend in real project
func ErrCannatListEntity(entity string, err error) *AppError {
